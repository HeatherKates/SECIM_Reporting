...
# Function to plot ellipse and points for each target
plot_function <- function(target) {
...
fig <- plot_ly()
lapply(unique(PCA_DATA[[2]]), plot_function)
fig
```
...
fig <- fig %>% layout(title = paste("Significant changes by metabolite class\n(",contrasts[[i]],")"),
         ...
         #It would be better to set the range to min,max across all contrasts,
         #But that information is not available when each plot is drawn.
         yaxis = list(title = "",tick0=0, dtick=1,range = list(-max(data.frame(barplotminmax.vector)),max(data.frame(barplotminmax.vector)))),margin = list(l = 75, t = 150),width=900,height=600)
barplotlist[[i]] = ggplotly(fig) 
} else
{
...
if(p_type=="adj.p.value"){
  ...
  arrange(adj.p.value, Level) %>% slice_head(n=20) %>% select(row.mode) %>% unlist(),
...
```{r,network,eval=TRUE,warning=FALSE,message=FALSE,fig.dim=c(30,20)}
