...
PCA_DATA[] <- lapply(PCA_DATA, function(col) {
...
for(target in unique(PCA_DATA$X1)) {
  target_data <- as.data.frame(scores[PCA_DATA$X1 == target, 1:2])
  target_color <- colors[which(as.character(unique(PCA_DATA$X1)) == target)]
  ...
  traces_list <- append(traces_list, list(scatter_trace))
  ...
  traces_list <- append(traces_list, list(ellipse_trace))
}

# Plot all the traces using plot_ly
fig <- plot_ly()
for(trace in traces_list) {
  fig <- add_trace(fig, type = trace$type, mode = trace$mode, x = trace$x, y = trace$y, 
                   line = trace$line, fill = trace$fill, fillcolor = trace$fillcolor, 
                   opacity = trace$opacity, showlegend = trace$showlegend, hoverinfo = trace$hoverinfo, 
                   text = trace$text, name = trace$name, marker = trace$marker)
}
fig
```
...
network_data <- setNames(
  lapply(vars_created, function(x) {
    list(element1 = NULL, element2 = NULL, element3 = NULL)
  }),
  vars_created
)
