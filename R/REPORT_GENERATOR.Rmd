...
if(num_of_metadata==2){
 ...
 PCA_DATA <- data.frame(cbind(
   colnames(Client_Data_Download[["Pos.normalized.data"]][-num_of_metadata,] %>% select(Client_Data_Download_out[["metadata"]]$Sample.name)),
   
                       t(Client_Data_Download[["Pos.normalized.data"]][1:(num_of_metadata-1),(num_of_metadata+1):(num_samples+2)]),
                             t(rbind(
  Client_Data_Download[["Pos.normalized.data"]][-c(1:num_of_metadata),] %>% select(Client_Data_Download_out[["metadata"]]$Sample.name),
  Client_Data_Download[["Neg.normalized.data"]][-c(1:num_of_metadata),] %>% select(Client_Data_Download_out[["metadata"]]$Sample.name)))))
