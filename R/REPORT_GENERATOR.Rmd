...
for (i in c(1:length(contrasts))){
  ...
  updatemenus <- list(
    list(direction = "down",
         ...
         buttons = lapply(0:19, function(idx) {
           list(
             method = "update",
             args = list(list(visible = as.logical(1:20 == idx + 1)),
                         list(yaxis = list(title = "Peak intensity"))),
  ...
  m <- list(l = 220, r = 50, b = 50, t = 50)
  plot <- plot_ly(data=my.df, x = boxplot_var, color = ~Class, yaxis = list(title = "Peak intensity"))
  ...
  plot <- plot %>%
    layout(
      ...
      title = paste("Violin plot of peak intensities across ", Grouping_Variable),
      showlegend = FALSE, yaxis = list(title = "Peak intensity")
    ) %>%
    add_annotations(
      x = 'center', y = 1.05, text = paste("Top metabolites ", contrasts[[i]], sep=""),
      xref = "paper", yref = "paper",
      showarrow = FALSE
    ) %>%
