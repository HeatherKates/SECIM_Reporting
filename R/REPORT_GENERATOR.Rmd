...
`r if(test_type == "lmm") {"A linear mixed model was performed using the R package stats (R Core Team 2023) to determine whether there was a significant effect of variable Class on metabolite peak intensity. The model was `}`r lm_model`r if(test_type == "lmm") {" Post-hoc pairwise comparisons were performed using Estimated Marginal Means (aka Least-Squares Means) as implemented in the R package emmeans (R.V. Lenth 2023). Adjusted p-values are corrected using the FDR method of p-value correction."}`
