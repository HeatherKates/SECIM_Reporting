...
`r if(test_type=="nostats"){"For each compound, a fold-change analysis was performed using the R package stats (R Core Team 2023)"}`
...
```{r}
if(!test_type == "nostats"){
  cat("## Table of changed metabolites </span>\n")
} else {
  cat("## Table of significantly changed metabolites </span>\n") 
}
```
...
```{r}
if(!test_type == "nostats"){
  cat("# Visualizations of changed metabolites</span>\n")
} else {
  cat("# Visualizations of significantly changed metabolites</span>\n") 
}
```
...
```{r}
if(!test_type == "nostats"){
  cat("## Barplots of >1.5 log2FC changes by metabolite class </span>\n")
} else {
  cat("## Barplots of significant changes by metabolite class </span>\n") 
}
```

```{r}
if(!test_type == "nostats"){
  cat("Hover over bars to view changed metabolites for each class. (If a contrast is missing, there were no >1.5 log2FC changed metabolites with KEGG IDs for that contrast) </span>\n")
} else {
  cat("Hover over bars to view significantly changed metabolites for each class. (If a contrast is missing, there were no significantly changed metabolites with KEGG IDs for that contrast) </span>\n") 
}
```
...
#Add colnames
if(test_type=="nostats"){
  colnames(bar.df) <- c("Class","Increase","Decreases","Increase_Text","Decrease_Text")
  if(sum(as.numeric(bar.df$Increase)+as.numeric(bar.df$Decreases))>0){
fig <- plot_ly()
fig <- fig %>% add_bars(
    x = bar.df$Class,
    y = as.numeric(bar.df$Decrease),
...
cat("## Plots of metabolites' fold-change and adjusted p-value for each contrast</span>\n")

cat("Hover over plot points to view compound labels.\n")

cat("Metabolites that pass the significance cutoff (but not fold-change) are orange, metabolites that pass the fold-change cutoff (but not significance) are black, and metabolites that pass both the fold-change and significance cutoffs are red. Metabolites in gray do not pass the significance or fold-change cutoff. If a color is missing from the plot, that is because no metabolites fall in that category. Adjusted p-value cutoff is 0.05, Fold-Change cutoff is 1.5.\n")

cat("```{r,volcano,echo=FALSE,warning=FALSE,message=FALSE,eval=TRUE}\n")
cat('volcanoplotlist = list()')
cat('for (i in 1:length(contrasts)){\n')
cat('## Volcano Plot\n')
cat('volcano.res <- Client_Data_Download[["report_results"]] %>% filter(contrast==contrasts[[i]])\n')
cat('#### Volcano plot of metabolites\' fold-change highlights metabolites with different abundances between groups\n')
cat('# Hover over points to view significantly metabolite names\n')
cat('# add a grouping column; default value is "not significant"\n')
cat('volcano.res$group <- "NotSignificant"\n')
cat('\n')
cat('# change the grouping for the entries with significance but not a large enough log2FC change\n')
cat('volcano.res[which(volcano.res$compound %in% sig_peaks[[i]] & volcano.res$row.ID %in% sig_peaks_rowID[[i]] & abs(volcano.res[\'log2FC\']) < 0.58 ),"group"] <- "Significant"\n')
cat('\n')
cat('# change the grouping for the entries a large enough log2FC change but not a low enough p value\n')
cat('volcano.res[which(abs(volcano.res[\'log2FC\']) > 0.58 ),"group"] <- "log2FC"\n')
cat('\n')
cat('# change the grouping for the entries with both significance and large enough log2FC change > 0.58\n')
cat('volcano.res[which(volcano.res$compound %in% sig_peaks[[i]]  & abs(volcano.res[\'log2FC\']) > 0.58 ),"group"] <- "Significant&log2FC"\n')
cat('\n')
cat('# make the Plot.ly plot\n')
cat('p <- plot_ly(data = volcano.res, x = volcano.res$log2FC, y = -log10(volcano.res[[p_type]]),hoverinfo = "text", text = paste(volcano.res$compound,volcano.res$Level,sep=":"),\n')
cat('             mode = "markers", color = volcano.res$group,colors = c(log2FC="black",NotSignificant="gray",Significant="orange",`Significant&log2FC`="red")) %>% \n')
cat('  layout(title=paste("P-value vs. Fold-Change in",str_split(contrasts[[i]],pattern="-")[[1]][1],"relative to\n",\n')
cat('                            str_split(contrasts[[i]],pattern="-")[[1]][2]),\n')
cat('         xaxis = list(title = \'log2FC\'), \n')
cat('         yaxis = list(title = \'-log10PVal\'),margin = list(l = 75, t = 150))\n')
cat('\n')
cat('margin = list(l = 75, t = 150)\n')
cat('\n')
cat('volcanoplotlist[[i]]=p\n')
cat(' }\n')
cat('htmltools::tagList(setNames(volcanoplotlist, NULL))\n')
cat("```\n")
}
```
...
```{r}
if(!test_type == "nostats"){
  cat("## Per-contrast boxplot of any of the top changed metabolites across sample class</span>\n")
} else {
  cat("## Per-contrast boxplot of any of the top significantly changed metabolites across sample class</span>\n") 
}
```
...
```{r}
if(!test_type == "nostats"){
  cat("## Heatmaps of metabolites with greatest log2FC difference in abundance</span>\n")
} else {
  cat("## Heatmaps of metabolites with most significant difference in abundance</span>\n") 
}
```
...
```{r}
if(!test_type == "nostats"){
  cat("The most changed (log2FC) compounds input into the KEGG subnetwork analysis are shown with red squares (see key) and are drawn in their KEGG subnetwork. Hover over nodes to view annotation and to see the confidence of our identification of a particular input KEGG compound.\n(If no subnetworks are visible below, there were no >1.5 log2FC changed metabolites with KEGG IDs.)\n")
} else {
  cat("The significantly changed compounds input into the KEGG subnetwork analysis are shown with red squares (see key) and are drawn in their KEGG subnetwork. Hover over nodes to view annotation and to see the confidence of our identification of a particular input KEGG compound.\n (If no subnetworks are visible below, there were no significantly changed metabolites with KEGG IDs.)\n") 
}
```
