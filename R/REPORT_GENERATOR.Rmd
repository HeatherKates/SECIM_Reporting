...
The PI provided `r {nrow(Client_Data_Download$metadata %>% filter(!str_detect(Sample.name, "Pool")))}` `r SampleType`
...
plot_ellipse_function <- function(target) {
  target_data <- as.data.frame(scores[PCA_DATA[,3] == target, 1:2])
  target_color <- colors[which(unique(PCA_DATA[,3]) == target)]
...
plot_points_function <- function(target) {
  target_data <- as.data.frame(scores[PCA_DATA[,3] == target, 1:2])
  rownames(target_data) <- rownames(PCA_DATA)[PCA_DATA[,3]==target]
  target_color <- colors[which(unique(PCA_DATA[,3]) == target)]
...
invisible(lapply(unique(PCA_DATA[,3]), plot_ellipse_function))

# Second loop: Plot all points
invisible(lapply(unique(PCA_DATA[,3]), plot_points_function))
