...
`r if(subset) {paste("This report is an analysis of", nrow(Client_Data_Download$metadata %>% filter(!str_detect(Sample.name, "Pool"))), SampleType, "as shown below. Classes with 0 samples were included in batch-correction and normalization but omitted from statistical analysis.")}`
...
`r if(subset) {cat("**PCA of all study samples that were included in batch-correction and normalization prior to statistical analysis, colored by batch.**")}`
...
plot_pca <- function(data, show_legend = TRUE) {
  PCA_DATA <- t(rbind(data[["Neg"]][,3:(num_samples+2)],
                      data[["Pos"]][-(1:num_of_metadata-1),3:(num_samples+2)]))
...
`r if(subset) {cat("**PCA of samples included in statistical analysis, colored by group.**")}`
...
```{r,network,eval=TRUE,error=TRUE,warning=FALSE,message=FALSE,fig.dim=c(40,40)}
