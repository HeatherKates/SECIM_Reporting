...
#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
...
#In the console stack_size <- getOption("pandoc.stack.size", default = "8G")
#Change from Tina Ho's version: 
#Re-write if/trycatch for KEGG network to deal with cases where there are sig compounds but they're not in #the FELLA data
#Include an option to subset metadata to only samples included in the statistical analysis in report
...
The PI provided `r {nrow(Client_Data_Download$metadata)}` samples.
...
The results of statistical tests to identify significant metabolites are provided. In addition, we also include raw, feature-filtered, and normalized metabolomic intensity datasets. Please review the "Read Me" sheet included in the download for a detailed explanations of variables.
...
if(test_type=="t.test"){
Client_Data_Download_out[["README"]] <- read.csv("ttest.README.csv")
}else{
Client_Data_Download_out[["README"]] <- read.csv("ANOVA.emmeans.README.csv")}
...
```{r}
if(test_type=="t.test"){for (i in c(1:4,7:9)){
  tryCatch({
...
Multivariate and univariate statistical analysis were performed to assess clustering of samples based on metabolomic profiles and to identify metabolites with significant changes in abundance between groups or biological conditions.

Following univariate statistical analysis of all compounds detected in positive and negative ionization modes independently, the list of compounds were combined between negative and positive ionization modes and reduced to a single representative compound per likely metabolite based on p-value (lowest p-value compound retrained).
...
for (i in 1:length(contrasts)){
print(paste(length(sig_peaks[[i]]), "metabolites were significantly changed (adj. p value < 0.05) between",str_split(contrasts[[i]],"-")[[1]][[1]], "and",str_split(contrasts[[i]],"-")[[1]][[2]],sep=" "))
}
}
```

## Table of significantly changed metabolites </span>
...
```{r,warn=FALSE}
if(p_type=="p.value"){
datatable(Client_Data_Download[["report_results"]] %>% filter(p.value<0.05) %>% dplyr::select(c(compound,p.value,Level,contrast,contains("FC"))) %>% dplyr::mutate(across(where(is.numeric), round, 5)),  caption = 'Table of significantly changed metabolites for (p-value < 0.05)')}else{
...
# Visualizations of significantly changed metabolites</span>
...
## Barplots of significant changes by metabolite class
...
#Add colnames
colnames(bar.df) <- c("Class","Significant_Increase","Significant_Decreases","Increase_Text","Decrease_Text")
#Print each compound on its own line
#bar.df$Increase_Text <- gsub(":","\n",bar.df$Increase_Text)
#bar.df$Decrease_Text <- gsub(":","\n",bar.df$Decrease_Text)
if(sum(as.numeric(bar.df$Significant_Increase)+as.numeric(bar.df$Significant_Decreases))>0){
fig <- plot_ly()
fig <- fig %>% add_bars(
    x = bar.df$Class,
    y = as.numeric(bar.df$Significant_Decrease),
...
fig <- fig %>% add_bars(
    x = bar.df$Class,
    y = as.numeric(bar.df$Significant_Increase),
    base = 0,
...
## Plots of metabolites' fold-change and adjusted p-value for each contrast</span>

Hover over plot points to view compound labels. 

Metabolites that pass the significance cutoff (but not fold-change) are orange, metabolites that pass the fold-change cutoff (but not significance) are black, and metabolites that pass both the fold-change and significance cutoffs are red. Metabolites in gray do not pass the significance or fold-change cutoff. If a color is missing from the plot, that is because no metabolites fall in that category. Adjusted p-value cutoff is 0.05, Fold-Change cutoff is 1.5.

```{r,volcano,echo=FALSE,warning=FALSE,message=FALSE,eval=TRUE}
...
## Per-contrast boxplot of any of the top significantly changed metabolites across sample class</span>
...
Client_Data_Download[["report_results"]] %>% filter(contrast==contrast[[i]]) %>% filter(compound %in% sig_peaks[[contrasts[[i]]]]) %>% arrange(adj.p.value) %>% slice_head(n=20) %>% select(compound) %>% unlist(),sep="_")}else{
...
## Heatmaps of metabolites with most significant difference in abundance</span>
...
The significantly changed compounds input into the KEGG subnetwork analysis are shown with red squares (see key) and are drawn in their KEGG subnetwork. Hover over nodes to view annotation and to see the confidence of our identification of a particular input KEGG compound.
...
load("Reporting_Input/KEGG_network.RDATA")
```
...
```{r,network_up,eval=TRUE,warning=FALSE,message=FALSE,fig.dim=c(10,10)}
rm(up.plot)
analysis.up <- NULL
#Find the metabolites are significantly changed between cluster-SDHx samples and cluster-one samples
sig.up.KEGGs <- Client_Data_Download[["report_results"]] %>% filter(contrast==contrasts[[i]]) %>% filter(compound %in% sig_peaks[[contrasts[[i]]]]) %>%
                                  filter(log2FC > 0) %>% 
                                  dplyr::select(KEGG) %>% filter(!KEGG=="")
###Up
if (nrow(sig.up.KEGGs)>0) {
tryCatch({
  analysis.up <- enrich(
  compounds = sig.up.KEGGs$KEGG,
  data = fella.data,
  method = "diffusion",
  approx = "normality")},error=function(cond) {
            message("Error")
...
if (!is.null(analysis.up)) {
#extract the sub-network 
up.graph <- generateResultsGraph(
    object = analysis.up,
    data = fella.data,
    method = "diffusion",
    ...
    threshold= 0.05,
    nlimit=250)
if(length(up.graph)>0){

vs <- V(up.graph) #Get node list - this has names
es <- as.data.frame(get.edgelist(up.graph)) # Get edgelist
#This has the names and whether it was input
node.data<-get.data.frame(up.graph,what="vertices") # Get node attribute dataframe
#Add shape data
node.data$input <- as.character(node.data$input)
node.data <- node.data %>%
...
node.data$com <- stri_replace_all_regex(node.data$com,pattern=as.character(c(1:6)),replacement=c("pathway","module","enzyme","reaction","compound","Significantly changed compound"),vectorize=FALSE)
Nv <- length(vs) #number of nodes
Ne <- length(es[1]$V1) #number of edges

#Coordinates for nodes
L <- layout.fruchterman.reingold(up.graph)
Xn <- L[,1]
Yn <- L[,2]

#Add KEGG names as attribute to graph
#Merge the vs$names with the KEGG_map df
KEGG_map <- readRDS("KEGG_map.df.RData")
vs.names <- data.frame(vs$name)
vs.names$num <- as.numeric(rownames(vs.names))
vs.names.KEGG <- merge(vs.names,KEGG_map,by.x="vs.name",by.y="V1",all.x=TRUE)
vs.names.KEGG <- vs.names.KEGG[order(vs.names.KEGG$num), ]

#Add confidence flag
vs.names.KEGG.confidence <- data.frame(Client_Data_Download[["report_results"]]  %>% filter(contrast==contrast[[i]]) %>% select("KEGG","Level") %>% filter(!KEGG=="") %>% distinct())
vs.names.KEGG.confidence <- merge(vs.names.KEGG,vs.names.KEGG.confidence,by.x="vs.name",by.y="KEGG")
#Recode confidence
vs.names.KEGG.confidence <- vs.names.KEGG.confidence %>% mutate(Level=case_when(Level==1~"High Confidence ID",.default=as.character(Level)))
vs.names.KEGG.confidence <- vs.names.KEGG.confidence %>% mutate(Level=case_when(Level==3~"Low Confidence ID",.default=as.character(Level)))
vs.names.KEGG.confidence$report.results.Level <- paste(vs.names.KEGG.confidence$V2,vs.names.KEGG.confidence$Level,sep=";")
#################
#################
...
vs.names.KEGG.confidence.2 <- vs.names.KEGG.confidence %>% filter(!vs.name %in% vs.names.KEGG.confidence[duplicated(vs.names.KEGG.confidence$vs.name),]$vs.name) 
vs.names.KEGG.confidence <- rbind(vs.names.KEGG.confidence.2,vs.names.KEGG.confidence %>% filter(vs.name %in% vs.names.KEGG.confidence[duplicated(vs.names.KEGG.confidence$vs.name),]$vs.name) %>% filter(Level=="High Confidence ID"))
#################
#################
vs.names.KEGG <- merge(vs.names.KEGG,vs.names.KEGG.confidence,all.x=TRUE)


vs.names.KEGG <- vs.names.KEGG %>% mutate(V2=case_when(!is.na(report.results.Level)~report.results.Level,.default=V2)) %>% dplyr::select(-report.results.Level)
vs.names.KEGG <- vs.names.KEGG[order(vs.names.KEGG$num), ]


#add linebreaks for long descriptions (hovertext) 
vs.names.KEGG$V2 <- gsub(";","\n",vs.names.KEGG$V2)

#Creates the nodes (plots the points)
network <- plot_ly(x = ~Xn, y = ~Yn, #Node points
                   mode = "markers", 
                   text = vs.names.KEGG$vs.name, 
                   hoverinfo = "text",
                   hovertext = vs.names.KEGG$V2,
                   color =as.factor(node.data$com),
                   colors=c(brewer.pal(5,"Dark2"),"red"),
                   symbol = ~node.data$input, symbols = c("circle","square")
                   )
network <- network %>% add_text(x = ~Xn, y = ~Yn,
                  text = vs.names.KEGG$vs.name)
...
names(Yn) <- names(vs)
for(j in 1:Ne) {
  v0 <- as.character(es[j,]$V1)
  v1 <- as.character(es[j,]$V2)
  ...
  edge_shapes[[j]] <- edge_shape
}

axis <- list(title = "", showgrid = FALSE, 
             showticklabels = FALSE, zeroline = FALSE)

#assign(x = paste("p2",i,sep="_"),
up.plot <- layout(
  network,
...
cat(paste("No KEGG subnetwork found for metabolites significantly more abundant in",str_split(contrasts[[i]],pattern="-")[[1]][1],"relative to\n",
                            str_split(contrasts[[i]],pattern="-")[[1]][2]))}
#networklist[[i]] <- 
if (exists("up.plot")) {
up.plot
...
#}
#}
#htmltools::tagList(setNames(networklist, NULL))
```

```{r,network_down,eval=TRUE,warning=FALSE,message=FALSE,fig.dim=c(10,10)}
rm(down.plot)
analysis.down <- NULL
sig.down.KEGGs <- Client_Data_Download[["report_results"]] %>% filter(contrast==contrasts[[i]]) %>% filter(compound %in% sig_peaks[[contrasts[[i]]]]) %>%
                                  dplyr::select(KEGG) %>% filter(!KEGG=="")
###Up
if (nrow(sig.down.KEGGs)>0) {
tryCatch({
  analysis.down <- enrich(
  compounds = sig.down.KEGGs$KEGG,
  data = fella.data,
  method = "diffusion",
  approx = "normality")},error=function(cond) {
            message("Error")
...
if (!is.null(analysis.down)) {
#extract the sub-network 
down.graph <- generateResultsGraph(
    object = analysis.down,
    data = fella.data,
    method = "diffusion",
    thresholdConnectedComponent = 0.05,
    threshold= 0.05,
...
if (length(down.graph)>0) {

vs <- V(down.graph) #Get node list - this has names
es <- as.data.frame(get.edgelist(down.graph)) # Get edgelist
#This has the names and whether it was input
node.data<-get.data.frame(down.graph,what="vertices") # Get node attribute dataframe
#Add shape data
node.data$input <- as.character(node.data$input)
node.data <- node.data %>%
  mutate(com = case_when(
...
#Coordinates for nodes
L <- layout.fruchterman.reingold(down.graph)
Xn <- L[,1]
Yn <- L[,2]
...
cat(paste("No KEGG subnetwork found for metabolites significantly less abundant in",str_split(contrasts[[i]],pattern="-")[[1]][1],"relative to\n",
                            str_split(contrasts[[i]],pattern="-")[[1]][2]))}
#networklist[[i]] <- 
if (exists("down.plot")) {
down.plot
...
```{r, ref.label=I('network_up'),eval=FALSE}
```

```{r, ref.label=I('network_down'),eval=FALSE}
```
